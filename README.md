...
Then we had to identify functional domains in the insulin protein family using InterPro/Pfam. So using the P number we found in Uniprot (P01308) I SEARCH ON InterPro about insulin. Then find the Family membership, and then click on similar proteins, clicked on the family and found the names of similar proteins. Then searched again that protein + human  and clicked on IGF1. then compaired the graaph of isnulin vs IGF to answer the questions. saw that even if tehy have different funtions they belong to same family , suggesting they came from common ancestral gene that later may have duplicated , got changes and then had a seperate function. 
Then wanted to see 3D structure of insulin and relate structure to function. Go to RCSB Protein Data Bank (RCSB PDB)  (https://www.rcsb.org). Searched P01308, the X-ray differentiation filter in Homo saprience (on the left). 
